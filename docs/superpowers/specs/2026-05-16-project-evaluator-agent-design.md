...
| 평가 검증 | `evaluation-critic` 서브에이전트 **항상 실행**(매 평가의 마지막 단계). 그 앞에 0토큰 결정적 인용 사전검사 |
| 비용 표시 | 매 실행 후 서브에이전트별 토큰/비용 내역을 TUI에 출력(LangChain usage 메타데이터 기반) |
| 관측(observability) | **LangSmith, 기본 ON**. deepagents=LangGraph라 env만으로 전체 실행 트리(메인→`task` 서브에이전트→툴/모델 호출, 토큰·지연·에러) 자동 트레이싱. `LANGSMITH_TRACING`(기본 true)·`LANGSMITH_API_KEY`·`LANGSMITH_PROJECT` env로 제어, 끄려면 `LANGSMITH_TRACING=false`. **주의: 트레이스에 평가 대상 레포 소스 스니펫·프롬프트가 LangSmith 클라우드로 전송됨**(사용자 본인 결정으로 기본 on). Langflow는 시각적 *저작* 도구라 관측 목적에 부적합 |
...
- LangSmith 트레이싱 기본 활성(`config.py`가 env 기본값 주입). `LANGSMITH_API_KEY` 없으면 크래시 대신 경고 후 트레이싱 비활성으로 진행

## 4. 서브에이전트 (4개, deepagents `subagents` dict)
...
- 역할: 주요 기술/패턴별로 (a) 목적부합 (b) 관용적 정확성 (c) 과/소설계를 판정
- **근거 계약(강제)**: 모든 주장은 구조화된 근거를 동반해야 함 — `{claim, file_path, line_start, line_end, quoted_snippet, rationale}`. 근거 없는 주장 금지(critic·사전검사가 검증할 대상이므로 형식 고정)
...
### 4.4 `evaluation-critic` (평가 검증) — 항상 실행

매 평가의 **마지막 단계**로 메인이 무조건 호출. 2계층 검증:

- **0계층 — 결정적 사전검사 (LLM 0토큰, 수 밀리초)**: tech-fit가 낸 각 근거에 대해 ① 인용 파일이 워크스페이스에 실재 ② `line_start..line_end`가 파일 범위 내 ③ `quoted_snippet`이 해당 위치 실제 내용과 일치하는지 검사. 불일치 = 환각/낡은 참조로 플래그
- **1계층 — LLM critic**: 인용된 파일 subset을 **독립적으로 다시 읽어**(tech-fit 추론 사슬에 비종속) 각 주장이 근거로 실제 정당화되는지, 과/소설계 판정에 명시적 정당화가 있는지, 약한·무근거·과신 주장을 골라냄
- 산출: 검증된 주장 / 수정 필요 주장 / 기각 주장 목록 + 보정된 평가. 메인은 이를 반영해 최종 리포트·DB 기록을 확정
- 도구: 0계층 결정적 인용검사 도구, 코드 리드(`read_file`, grep)
- 컨텍스트 격리: 별도 서브에이전트라 독립 컨텍스트에서 검증(상관 오류 완화)
...
URL → 메인이 `write_todos` 계획 → `recon` 클론(commit SHA 기록)·지도 작성 → `dependency-stack` 매니페스트 파싱 → `tech-fit` 파일/기술별 평가(근거 계약 준수) → **`evaluation-critic` 검증(항상): 0계층 인용검사 → 1계층 LLM critic** → 메인이 critic 결과 반영해 전체 종합 → `record_evaluation` 저장 → TUI에 리포트 + **서브에이전트별 토큰/비용 내역** 렌더 → 후속 Q&A.

### 6.2 재실행 (같은 URL)

`get_repo_history` + `diff_since_last` → `recon`이 워크스페이스 갱신(`git pull`, 기본 최신 HEAD, ref 지정 가능) → **변경/추가 파일만** recon 지도 갱신 + `tech-fit` 재평가, **삭제** 파일은 제거, **안 바뀐** 파일은 저장된 판정 재사용 → **`evaluation-critic`이 새로/재평가된 주장만 검증(항상)** → 메인이 **델타 리포트** 생성(무엇이 바뀜 / 재평가 결과 / 전체 평가가 어떻게 이동) → commit 참조하는 새 평가 행 기록.
...
| DB | 시작 시 버전드 스키마 초기화 |
| LangSmith 키 없음(기본 on인데 미설정) | 크래시 금지 — 경고 1회 출력 후 트레이싱 비활성으로 정상 진행 |
...
- 단위(네트워크 없음, 로컬 임시 git 픽스처):
  ...
  - 프로젝트 지도 빌더(스킵 규칙·상한)
  - **0계층 인용 사전검사**(LLM 0): 실재/범위/스니펫 일치 — 정상·실재안함·범위초과·스니펫불일치 케이스 픽스처
  - 서브에이전트별 토큰/비용 집계(usage 메타데이터 → 내역 표)
- 서브에이전트 판정: 특성 알려진 소형 픽스처 레포(예: 의도적으로 과설계한 토이 레포)로 verdict가 오버엔지니어링을 지적하는지 검증. **critic이 무근거/환각 인용 주장을 기각하는지** 검증. LLM 의존 테스트는 최소화·플래그 처리
...
src/project_evaluator/
  ...
  subagents/
    ...
    dependency_stack.py
    tech_fit.py
    evaluation_critic.py   # 항상 실행되는 검증 서브에이전트
  tools/
    ...
    citation_check.py # 0계층 결정적 인용 사전검사 (LLM 0)
    usage.py          # 서브에이전트별 토큰/비용 집계·표시
  db/
    schema.sql
    store.py
tests/
...
의존성: `deepagents`, `langchain-openai`, `langsmith`, `gitpython`, `rich`, `pydantic`, `pytest`.
